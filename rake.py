...
def splitSentences(text):
	sentenceDelimiters = re.compile('[^a-zA-Z0-9_\\+\\-/]')#re.compile(u'[.!?,;:\t\\-\\"\\(\\)\\\'\u2019\u2013]')
...
def launch_re(text, stoplistname):
    sentenceList = splitSentences(text)
    stoppath = stoplistname #Fox stoplist contains "numbers", so it will not find "natural numbers" like in Table 1.1
